...
/**
 * CreateDefaultCharacterPage creates a default character in the previously selected world without having to select stats or items.
 * @param {object} worldToView
 * @returns UI for CreateDefaultCharacterPage
 */
export const CreateDefaultCharacterPage = ({ worldToView }) => {
    const [character_name, setCharacterName] = useState(null);
    //const [world_name, setHomeworldName] = useState(null);
    const navigate = useNavigate();

    // adds the default character using a POST http request
    const addCharacter = async () => {
        const world_name = worldToView.world_name
        const newCharacter = { world_name, character_name };
        const response = await fetch('/worlds/characters/create_default_character', {
            method: "POST",
                ...
                <nav>
                    <ul>
                        ...
                        <li class="active_inactive_link">
                            <span>Create Default Character</span>
                        </li>
...
export default CreateDefaultCharacterPage
